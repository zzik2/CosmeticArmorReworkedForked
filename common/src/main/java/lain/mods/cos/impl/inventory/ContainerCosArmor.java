...
public class ContainerCosArmor extends RecipeBookMenu<CraftingInput, CraftingRecipe> {
        ...
        for (int k = 0; k < 4; ++k) {
            ...
            addSlot(new Slot(invPlayer, 39 - k, 8, 8 + k * 18) {
                ...
                public boolean mayPlace(ItemStack stack) {
                    return equipmentslottype.getType() == EquipmentSlot.Type.HUMANOID_ARMOR
                            && player.getEquipmentSlotForItem(stack) == equipmentslottype;
                }
    ...
    private static void updateCrafting(AbstractContainerMenu menu, Level world, Player player,
        ...
        if (!world.isClientSide) {
            ...
            Optional<RecipeHolder<CraftingRecipe>> optional = world.getServer().getRecipeManager()
            ...
            serverplayer.connection
                    .send(new ClientboundContainerSetSlotPacket(menu.containerId, menu.incrementStateId(), 0, stack));
        }
    }
    ...
    public ItemStack quickMoveStack(Player player, int slotIndex) {
        ...
        if ((slot != null) && (slot.hasItem())) {
            ...
            } else if ((slotIndex >= 46) && (slotIndex < 50)) // CosmeticArmor
            {
                if (!moveItemStackTo(stack1, 9, 45, false))
                    return ItemStack.EMPTY;
            } else if (desiredSlot.getType() == EquipmentSlot.Type.HUMANOID_ARMOR
                    && !slots.get(8 - desiredSlot.getIndex()).hasItem()) // ItemArmor - check NormalArmor slots
            {
                ...
                if (!moveItemStackTo(stack1, j, j + 1, false))
                    ...
                    && !slots.get(49 - desiredSlot.getIndex()).hasItem()) // ItemArmor - check CosmeticArmor slots
            {
